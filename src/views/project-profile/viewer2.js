...
export default class Viewer {
  ...
  setCamera(index) {
    ...
    this.bloomComposer.camera = camera
    this.labelRenderer.camera = camera
  }
  /**
   * @function: 自适应窗口
   */
  ...
  loadServiceArea() {
    ...
    this.gltfLoader.load(
      ...
      (gltf) => {
        ...
        this.serviceArea = gltf
        this.gltfScene = gltf.scene
        this.scene.add(gltf.scene)
        gltf.scene.animations.push(...gltf.animations)
        this.mixer = new THREE.AnimationMixer(this.scene)
        this.setCamera(0) // 假设使用第一个相机
        this.action = '空物体.001Action'
        ...
        gltf.scene.traverse((child) => {
          if (child.isMesh) {
            ...
            if (['充电桩a', '充电桩b'].includes(child.name)) {
              ...
              sprite.scale.set(0.4, 0.4, 0.4) // 设置精灵大小
  ...
  set action(name) {
    ...
    if (clip !== undefined) {
      ...
      if (name == '空物体.001Action') {
        action.clampWhenFinished = false
        action.setLoop(THREE.LoopRepeat)
        action.timeScale = 0.4 // 将动画速度设为原来的一半
      } else {
